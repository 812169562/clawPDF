...
namespace DrawTools
{
    ...
    public sealed class DrawingCanvas : Canvas
    {
        ...
        {
            var a = this.Zoom;
            if (this.visuals.Count() == 0) return;
            isAll = false;
            var visual = GetVisualChild(this.visuals.Count() - 1);
            visuals.Remove(visual);
        ...
        public void Redo()
        {
            ...
            if (isAll)
            {
                foreach (Visual visual in undoVisuals)
                {
                    AddVisual(visual);
                }
                undoVisuals = new List<Visual>();
            }
            else
            {
                var visual = undoVisuals[this.undoVisuals.Count() - 1];
                AddVisual(visual);
        ...
        /// <param name="image"></param>
        /// <returns></returns>
        public BitmapFrame ToBitmapFrame(BitmapSource image)
        {
            var dpi = new Dpi(image.DpiX, image.DpiY);
            var visual = GetDrawingVisual(image.PixelWidth, image.PixelHeight, dpi, image);

            if (visual == null)
                return null;

            var renderBitmap = new RenderTargetBitmap(image.PixelWidth, image.PixelHeight, dpi.DpiX, dpi.DpiY, PixelFormats.Pbgra32);
            renderBitmap.Render(visual);
        ...
        public bool UndoEnabled => this.visuals.Count() > 0;
        public bool RedoEnabled => this.undoVisuals.Count() > 0;
